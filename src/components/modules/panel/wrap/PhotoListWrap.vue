...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      if (state.types.length <= 0) {
        ...
        for (const iterator of types) {
          const { list } = await api.material.getImagesList({ cate: iterator.id, per_page: 2 })
    ...
    const getDataList = async () => {
      ...
      loading = true
      state.page += 1
      let { list = [], total } = await api.material.getImagesList({ cate: state.currentCategory.id, page: state.page, per_page: 30 })
