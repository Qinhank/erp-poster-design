...
-->
<template>
  <div v-if="!noMenu" class="top-title"><el-input v-model="title" placeholder="未命名的设计" class="input-wrap" /></div>
  ...
  setup(props, context) {
    ...
    async function saveTemp() {
      ...
      if (type == 1) {
        ...
        res = await api.home.saveTemp({ id: tempid, cover_base64: image, type, title: proxy.title || '未命名组件', content: JSON.stringify(proxy.dWidgets), width: proxy.dPage.width, height: proxy.dPage.height })
      } else {
        res = await api.home.saveTemp({ id: tempid, cover_base64: image, title: proxy.title || '未命名模板', content: JSON.stringify({ page: proxy.dPage, widgets: proxy.dWidgets }), width: proxy.dPage.width, height: proxy.dPage.height })
      }
